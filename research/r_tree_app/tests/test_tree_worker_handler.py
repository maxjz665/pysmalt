...
class TestTreeWorkerHandler(TestCase):
    ...
    async def test_generate_features(self):
        ...
        items = TreeWorkerHandler._generate_features(pos, sector_size=0, many_sectors=True, is_need_uno=True, is_need_duo=True, is_need_separate=False)
        self.assertEqual(len(items), len(pos) ** 2 + len(pos))
        self.assertIn("часть1-часть2", items)

        # проверка генерации пар с одним поворотом
        items = TreeWorkerHandler._generate_features(pos, sector_size=25, many_sectors=False, is_need_uno=True, is_need_duo=True, is_need_separate=True)
        ...
        items = TreeWorkerHandler._generate_features(pos, sector_size=25, many_sectors=True, is_need_uno=True, is_need_duo=True, is_need_separate=True)
        ...
        items = TreeWorkerHandler._generate_features(pos, sector_size=25, many_sectors=True, is_need_uno=False, is_need_duo=False, is_need_separate=True)
        self.assertEqual(len(items), len(pos) ** 2 * (2 * 3)) # исходные N^2 пары + пары пар с плюсом и с минусом
        self.assertIn("часть1(0.5)-часть3(0.5)", items)
        self.assertIn("часть1(0.5)+часть3(0.5)", items)
    ...
    async def test_generate_slice(self):
        ...
        self.assertListEqual(items[0][:18], [1,2,3,4,5,6,7,8,9, 1*0.25+1*0.75, 1*0.25+2*0.75, 1*0.25+3*0.75, 1*0.25+4*0.75, 1*0.25+5*0.75, 1*0.25+6*0.75, 1*0.25+7*0.75, 1*0.25+8*0.75, 1*0.25+9*0.75])
