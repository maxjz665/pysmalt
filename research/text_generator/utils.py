...
def generate_text_by_code(code: CodeGen, base_words: List[TextWord], other_words: List[TextWord]) -> List[
                                                                                                         GeneratedWord] | None:
    """
...
def export_parsing(generated_text: List[GeneratedWord]) -> str:
    ...
    for item in generated_text:
        if item.word.word != '|':
            ...
            if item.word.params:
                params = [getattr(item.word.params, f'param_{i:02d}', 0) for i in
                          range(1, 21)] if item.word.params else [0] * 20
            else:
                params = [0] * 20
